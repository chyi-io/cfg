...
Deno.test("wsUrlToHttp: ws → http (regression: must not become https)", () => {
  assertEquals(wsUrlToHttp("ws://localhost:8001/api/agent/ws"), "http://localhost:8001/api/agent/ws");
});

Deno.test("wsUrlToHttp: wss → https", () => {
  assertEquals(wsUrlToHttp("wss://cfg.chyi.io/api/agent/ws"), "https://cfg.chyi.io/api/agent/ws");
});
...
Deno.test("stripAgentWsPath: leaves unrelated paths alone", () => {
  ...
  assertEquals(stripAgentWsPath("https://cfg.chyi.io/foo/bar"), "https://cfg.chyi.io/foo/bar");
});
