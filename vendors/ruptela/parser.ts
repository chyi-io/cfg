 ...
 */
// Internal synchronous body. The exported Promise-returning wrapper below
// matches the VendorPlugin.parse signature; keeping the heavy lifting
// synchronous avoids the `require-await` lint error on functions that don't
// actually await anything.
const parseRuptelaConfigSync = (buffer: Uint8Array): ParseResult => {
  try {
    ...
    if (!hasHeader && !lines[0].includes("=")) {
      return {
        success: false,
        error: "Not a recognized Ruptela config format",
      };
    }
...
export const parseRuptelaConfig = (
  buffer: Uint8Array,
): Promise<ParseResult> => Promise.resolve(parseRuptelaConfigSync(buffer));
 ...
 * @returns File bytes.
 */
const generateRuptelaConfigSync = (
...
export const generateRuptelaConfig = (
  config: Record<string, string>,
): Promise<Uint8Array> => Promise.resolve(generateRuptelaConfigSync(config));
