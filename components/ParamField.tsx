...
const ParamField = (
  { paramId, meta, value, error, onValueChange }: ParamFieldProps,
) => {
  ...
  return (
    <div class={`group ${disabled ? "opacity-60" : ""}`}>
      ...
      {hasOptions
        ? (
          <select
            ...
            disabled={disabled}
            onChange={(e) =>
              onValueChange(paramId, (e.target as HTMLSelectElement).value)}
            class={`w-full ${borderClass} border rounded-lg px-3 py-2 text-sm ${
              disabled ? "" : "bg-gray-50 focus:bg-white"
        ...
        : (
          <input
            ...
            disabled={disabled}
            onInput={(e) =>
              onValueChange(paramId, (e.target as HTMLInputElement).value)}
            class={`w-full ${borderClass} border rounded-lg px-3 py-2 text-sm ${
              disabled ? "" : "bg-gray-50 focus:bg-white"
            } focus:outline-none focus:ring-2 focus:border-transparent transition-colors`}
      ...
      {error && (
        <p class="text-[11px] text-red-500 mt-1 leading-tight font-medium">
          {error}
        </p>
      )}
