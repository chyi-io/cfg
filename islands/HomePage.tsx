...
export default function HomePage() {
  ...
  const handleFileSelect = async (file: File) => {
    ...
    try {
      ...
      globalThis.location.href = `/config/${data.vendorId}/${data.deviceId}`;
    } catch (_err) {
  ...
  const handleCreateNew = async (vendorId: string, deviceId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/defaults?vendor=${vendorId}&device=${deviceId}`,
      );
      if (!response.ok) {
        const err = await response.json();
      ...
      const data = await response.json();
      sessionStorage.setItem("hw-config", JSON.stringify(data));
      globalThis.location.href = `/config/${vendorId}/${deviceId}`;
    } catch (_err) {
      ...
      <div class="container mx-auto px-4 py-12 max-w-5xl">
        ...
        <div class="text-center mb-10">
          ...
          <p class="text-gray-500 text-lg max-w-xl mx-auto">
            Edit config files offline, or talk to a real device on your network
            in real time.
          </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-12">
          <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <p class="text-sm text-gray-600 mb-3">
              Upload a Teltonika, Ruptela, or Chafon config file. Edit
              parameters in a structured UI. Download the result.
            </p>
            <p class="text-xs text-gray-400">
              No device required — works fully offline in your browser.
            </p>
          </div>
          ...
          >
            <div class="flex items-center gap-3 mb-2">
              ...
              <span class="ml-auto text-emerald-600 group-hover:translate-x-1 transition-transform">
                →
              </span>
            </div>
            <p class="text-sm text-gray-600 mb-3">
              Read and write live device parameters over the network: work mode,
              remote server, whitelist, more.
            </p>
            <p class="text-xs text-emerald-700 font-medium">
              Requires the chyi-cfg-agent installed on a Linux box on the
              device's LAN.
            </p>
          </a>
        </div>
        ...
        <div class="mb-6 flex items-center gap-3">
          <h2 class="text-lg font-bold text-gray-800 flex-shrink-0">
            Create New
          </h2>
        ...
        <footer class="mt-14 flex flex-col items-center gap-3 text-xs text-gray-400">
          <div class="flex items-center gap-4">
            <a
              href="/docs"
              class="flex items-center gap-1.5 text-gray-500 hover:text-blue-600 transition-colors"
            >
              <svg
                class="w-4 h-4"
                fill="none"
            ...
            <a
              href="https://github.com/chyi-io/cfg"
              target="_blank"
          ...
          <p>
            ...
            <a
              href="https://github.com/mutasimissa"
              target="_blank"
              rel="noopener noreferrer"
