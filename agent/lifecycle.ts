...
export function statusCmd(): number {
  ...
  console.log(
    `  ${term.gray("ffi        ")} ${shouldUseMock() ? "mock" : "native"}`,
  );
  console.log(
    `  ${term.gray("drivers    ")} ${
      listDrivers().map((d) => d.id).join(", ")
    }`,
  );
  return 0;
}
  ...
  if (!shouldUseMock()) {
    try {
      ...
      results.push({
        ...
        hint:
          "Native libs should auto-extract from the bundled binary. Check that the cache dir (CHYI_CFG_AGENT_HOME or ~/.cache/chyi-cfg-agent) is writable. Last-resort override: set CHYI_CFG_AGENT_LIB_PATH to a custom libCFApi.so.",
      });
    }
  } else {
    results.push({
      check: "ffi mode",
      ok: true,
      detail: "mock (CHYI_CFG_FFI=mock)",
    });
  }

  try {
    const base = new URL(stripAgentWsPath(wsUrlToHttp(cfg.cloudUrl)));
    const res = await fetch(new URL("/api/live/health", base), {
      method: "GET",
    });
    if (res.ok) {
      ...
      results.push({
        ...
        ok: true,
        detail: `HTTP ${res.status}, server ${body.version ?? "?"} proto ${
    ...
    results.push({
      ...
      detail: err instanceof Error ? err.message : String(err),
      hint:
        "Check that CHYI_CFG_URL points to a reachable cloud and that outbound 443 is open.",
    });
  }
...
export async function pairCmd(): Promise<number> {
  ...
  console.log(
    term.gray(`  expires ${new Date(result.expiresAt).toLocaleString()}`),
  );
  return 0;
}
...
async function connectAndAwaitPairCode(
...
> {
  return await new Promise((resolve) => {
    ...
    const timeout = setTimeout(
      () => settle({ ok: false, error: "timeout after 10s" }),
      10000,
    );
    ...
    ws.onmessage = async (ev) => {
      ...
      if (env.kind === "challenge") {
        ...
        ws.send(
          encode({ kind: "challenge_response", agentId: cfg.agentId, sig }),
        );
        return;
      }
      if (env.kind === "ack") {
        ...
        if (env.pairCode && env.pairCodeExpiresAt) {
          settle({
            ok: true,
            code: env.pairCode,
            expiresAt: env.pairCodeExpiresAt,
          });
        } else {
...
export function resetCmd(): number {
  ...
  try {
    ...
    if (err instanceof Deno.errors.NotFound) {
      ...
      console.error(
        term.fail(`remove failed: ${err instanceof Error ? err.message : err}`),
      );
      return 1;
    }
  }
...
export async function setupCmd(): Promise<number> {
  ...
  const url = prompt(`Cloud WebSocket URL [${defaultUrl}]:`)?.trim() ||
    defaultUrl;
  ...
  if (code === 0) {
    ...
    console.log(
      term.gray(
        "  next: chyi-cfg-agent run    (or systemctl --user start chyi-cfg-agent)",
      ),
    );
  } else {
    console.log(
      term.warn(
        "setup saved but some checks failed — fix the items above before `chyi-cfg-agent run`",
      ),
    );
  }
  return 0;
...
export async function logsCmd(): Promise<number> {
  ...
  try {
    ...
    console.error(
      term.gray(
        "  if running outside systemd, the agent logs to stdout — capture it there.",
      ),
    );
    return 1;
  }
...
export function helpCmd(): number {
  ...
  console.log(
    term.dim("  LAN bridge between the cfg cloud UI and a local device."),
  );
  console.log("");
  ...
  const daily: [string, string][] = [
    [
      "run",
      "(default, systemd runs this) connect to cloud + dispatch device commands",
    ],
    ["pair", "one-shot: connect, get a fresh pair code, exit"],
    [
      "status",
      "version, agentId, cloud URL, config path, FFI mode, driver list",
    ],
    ["doctor", "preflight checks (FFI dlopen, cloud reachability, glibc)"],
    [
      "logs",
      "follow the systemd journal (journalctl --user -u chyi-cfg-agent -f)",
    ],
  ];
  ...
  const lifecycle: [string, string][] = [
    ...
    [
      "update",
      "in-place update to the latest cloud version (stop → swap → restart)",
    ],
    [
      "reset",
      "rotate local agentId + agentKey (orphans existing browser tokens)",
    ],
    ["uninstall", "remove binary, libs, config, systemd unit (full cleanup)"],
  ];
  for (const [c, d] of lifecycle) {
    console.log(`  ${term.cyan(c.padEnd(10))} ${d}`);
  }
  console.log("");
  ...
  const flags: [string, string][] = [
    [
      "run --force",
      "start even if the systemd service is already running (expect conflicts)",
    ],
    ["update --force", "reinstall / allow downgrade (passed to install.sh)"],
    [
      "update --no-start",
      "update the binary + unit without (re)starting the service",
    ],
  ...
  const envs: [string, string][] = [
    [
      "CHYI_CFG_URL",
      "cloud WebSocket URL (default: wss://cfg.chyi.io/api/agent/ws)",
    ],
    [
      "CHYI_CFG_AGENT_HOME",
      "config + state dir (default: ~/.config/chyi-cfg-agent)",
    ],
    ["CHYI_CFG_LOG_LEVEL", "debug | info | warn | error (default: info)"],
    ["CHYI_CFG_FFI", "set to `mock` for in-memory dev (no hardware)"],
    [
      "CHYI_CFG_AGENT_LIB_PATH",
      "override path to libCFApi.so (default: auto-extracted)",
    ],
  ...
  console.log(
    term.gray(
      "  # in-place update (will stop the service, swap binary, restart)",
    ),
  );
  ...
  console.log(
    term.gray(
      "  # dev: run against a local cloud with the mock driver (no hardware needed)",
    ),
  );
  console.log(
    "  CHYI_CFG_URL=ws://localhost:8001/api/agent/ws CHYI_CFG_FFI=mock chyi-cfg-agent run",
  );
  console.log("");
...
export async function uninstallCmd(autoYes = false): Promise<number> {
  ...
  const installDir = Deno.env.get("CHYI_CFG_AGENT_HOME") ??
    `${home}/.local/share/chyi-cfg-agent`;
  ...
  console.log(
    `    ${installDir}        ${term.gray("(binary + extracted libs)")}`,
  );
  console.log(
    `    ${binSymlink}                ${term.gray("(symlink in PATH)")}`,
  );
  console.log(`    ${unitPath}        ${term.gray("(systemd user unit)")}`);
  console.log(`    ${cacheDir}        ${term.gray("(cached native libs)")}`);
  console.log(
    `    ${configLocation()}  ${term.gray("(agent identity + cloud URL)")}`,
  );
  console.log("");
  console.log(term.gray(
  ...
  const step = (label: string, fn: () => void | Promise<void>): Promise<void> =>
    Promise.resolve(fn()).then(
      ...
      (err) => {
        ...
        console.log(
          `  ${term.fail(label)} ${
            term.gray(err instanceof Error ? err.message : String(err))
          }`,
        );
      },
    );
